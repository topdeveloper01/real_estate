...
const City1Screen = (props) => {
	...
	const loadData = async () => {
		try {
			setLoading(true);
			let all_items = await getAllCities(1);
			setCities(all_items)
			setLoading(false);
		}
		catch (error) {
	...
	const onDeleteCity = async (city_item) => {
		try {
			setLoading(true);
			await city1_Collection.doc(city_item.id).delete()
			let city2_docs = await city2_Collection.where('city_1', '==', city_item.id).get();
			let city3_docs = await city3_Collection.where('city_1', '==', city_item.id).get();
			city2_docs.forEach(element => {
				element.ref.delete() 
			});  
			city3_docs.forEach(element => {
				element.ref.delete() 
			});  
							...
							<View style={[Theme.styles.col_center_start, { width: '100%', paddingHorizontal: 8, alignItems: 'flex-start', }]}>
								{
									cities.map((item) =>
										<TouchableOpacity 
											...
											onPress={() => {
												props.navigation.navigate(RouteNames.City2Screen, { city1_id: item.id , title : item.name })
											}}>

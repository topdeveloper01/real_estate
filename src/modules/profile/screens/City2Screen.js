...
const City2Screen = (props) => {
	...
	const loadData = async () => {
		try {
			setLoading(true);
			let all_items = await getAllCities(2);
			all_items = all_items.filter(item => item.city_1 == city1_id)
			setCities(all_items)
			setLoading(false);
		}
		catch (error) {
	...
	const onDeleteCity = async (city_item) => {
		try {
			setLoading(true);
			await city2_Collection.doc(city_item.id).delete()
			let city3_docs = await city3_Collection.where('city_2', '==', city_item.id).get();
							...
							<View style={[Theme.styles.col_center_start, { width: '100%', paddingHorizontal: 8, alignItems: 'flex-start', }]}>
								{
									cities.map((item) =>
										<TouchableOpacity 
											...
											onPress={() => {
												props.navigation.navigate(RouteNames.City3Screen, {city1_id : city1_id,  city2_id: item.id , title : item.name})
											}}>

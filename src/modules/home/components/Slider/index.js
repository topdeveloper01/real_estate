...
const Slider = (
  ...
  const [lowValue, setLowValue] = useState(inPropsRef.current.low);
  const [highValue, setHighValue] = useState(inPropsRef.current.high);
  ...
  const lowScalValue = (low - min) / fullScale;
  const highScalValue = (max - high) / fullScale;

  const [labelView, labelUpdate] = useThumbFollower(containerWidthRef, gestureStateRef, renderLabel, isPressed, allowLabelOverflow, lowScalValue);
  const [highlabelView, highLabelUpdate] = useThumbFollower(containerWidthRef, gestureStateRef, renderLabel, isPressed, allowLabelOverflow, highScalValue);
  const [notchView, notchUpdate] = useThumbFollower(containerWidthRef, gestureStateRef, renderNotch, isPressed, allowLabelOverflow, lowScalValue);
  const [highnotchView, highnotchUpdate] = useThumbFollower(containerWidthRef, gestureStateRef, renderNotch, isPressed, allowLabelOverflow, highScalValue);
    ...
    onPanResponderGrant: ({ nativeEvent }, gestureState) => {
      ...
      const handlePositionChange = positionInView => {
        ...
        if (isLow) {
          ...
          // highLabelUpdate && highLabelUpdate(gestureStateRef.current.lastPosition, value);
          setHighValue(value)
        }
        // if (isLow) {
        ...
        // }
        // else {
        //   highnotchUpdate && highnotchUpdate(gestureStateRef.current.lastPosition, value);
        // }
  ...
  }), [pointerX, inPropsRef, thumbWidth, disableRange, disabled, onValueChanged, setLow, setHigh, labelUpdate, highLabelUpdate, notchUpdate, highnotchUpdate, updateSelectedRail]);
